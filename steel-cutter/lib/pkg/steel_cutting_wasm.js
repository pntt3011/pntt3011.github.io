/* @ts-self-types="./steel_cutting_wasm.d.ts" */

/**
 * @param {any} items
 * @param {number} stock_length
 * @param {number} max_primary_patterns
 * @param {number} bundle_size
 * @returns {any}
 */
export function compute_cutting_plan(items, stock_length, max_primary_patterns, bundle_size) {
    const ret = wasm.compute_cutting_plan(items, stock_length, max_primary_patterns, bundle_size);
    if (ret[2]) {
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./steel_cutting_wasm_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('steel_cutting_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
